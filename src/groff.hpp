...
class groff {
 public:
  ...
  void operator()(parse_state &state) {
    ...
    auto const contents(std::find_if(
        state.lines.begin(), state.lines.end(), [](std::string const &l) {
          return l.find(".SH Contents") != std::string::npos;
        }));
    if (contents != state.lines.end()) {
      auto const next_section(std::find_if(
          contents + 1, state.lines.end(), [](std::string const &l) {
