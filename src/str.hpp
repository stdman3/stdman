...
namespace str {
inline void replace(std::string &str, std::string const &from,
                    std::string const &to) {
  if (from.empty() || str.empty()) {
    return;
  }

  for (size_t start_pos{};
...
inline std::string &ltrim(std::string &s) {
  auto const found(std::find_if_not(
      s.begin(), s.end(), [](char const c) { return std::isspace(c); }));
...
inline std::string &rtrim(std::string &s) {
  auto const found(std::find_if_not(
      s.rbegin(), s.rend(), [](char const c) { return std::isspace(c); }));
...
inline void erase(std::string &s, std::string const &begin,
